...
public class PageDto {
	...
	public PageDto(int total, Criteria criteria) {
		...
		if (realEndpage < this.endPage) {  // 실제 마지막 페이지 값(realEndpage)이 계산한 endpage 수보다 작을 때 실제 페이지 값으로 교체
			this.endPage = realEndpage;	
		}
		
		this.prev = this.startPage != 1;  // startPage 값이 1이 아니면 prev는 true 
		this.next = realEndpage > this.endPage;  // endPage 번호가 실제 페이지 번호보다 작은 경우에만 next가 true
